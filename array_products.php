<?php
    ...
    for ($i = 0; $i <= $count_data; $i++){
        ...
        for ($i2 = 0; $i2 <= $count_bands; $i2++) {
            ...
            } else if ( $member_band_id === 3390641849){
        ...
        for ($i = 0; $i <= $count_data; $i++) {
            if (isset($data->items[$i]->album_title) && $data->items[$i]->album_title === $album_title) {
                ...
                if($data->items[$i]->album_title === NULL):
                    ...
                    $album_title = $data->items[$i]->album_title;
                endif;                $arrayProducts[$album_title] = ['post_id' => $post_id, 'band_id' => $member_band_id, 'groupe' => $band_name,'product_name' => $album_title,'image_url'=> $image_url, 'variants' => [$title => ['format' => $title, 'prix' => $price]]];
                $post_id++;
            }
